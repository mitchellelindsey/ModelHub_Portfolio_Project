...
const createUser = async function (req, res) {
  try {
    ...
    if (!user)
      return res.status(400).json({ message: "User Account was not created" });
...
const logUser = async function (req, res) {
  try {
    ...
    if (!password) res.status(400).json({ message: "Password is invalid" });
...
module.exports = { createUser, logUser, userProfile };